...
class EvalVideoDataset(Dataset):
    ...
    def __init__(
        ...
        """
        metadata_df = (
            EvalVideoDataset._append_root_dir_to_filenames_and_check_files_exist(
                metadata_df, root_dir
            )
        )
        self.clip_metadata_df = EvalVideoDataset._generate_clips_metadata(
            metadata_df, clip_length, frame_rate, stride
        )
    ...
    def __getitem__(self, idx):
        ...
        filename, fps, clip_t_start, is_last_clip = (
            row["filename"],
            row["fps"],
            row["clip-t-start"],
            row["is-last-clip"],
        ...
        vframes, _, _ = read_video(
            filename=filename,
            start_pts=clip_t_start,
            end_pts=clip_t_end,
            pts_unit="sec",
        )
        idxs = EvalVideoDataset._resample_video_idx(
            self.clip_length, fps, self.frame_rate
        )
        vframes = vframes[idxs][
            : self.clip_length
        ]  # [:self.clip_length] for removing extra frames if isinstance(idxs, slice)
        if vframes.shape[0] != self.clip_length:
            raise RuntimeError(
                f"<EvalVideoDataset>: got clip of length {vframes.shape[0]} != {self.clip_length}."
    ...
    def save_output(self, batch_output, batch_input, label_columns):
        ...
        for i in range(batch_output[0].shape[0]):
            filename, is_last_clip = (
                batch_input["filename"][i],
                batch_input["is-last-clip"][i],
            )
            ...
            if is_last_clip:
                ...
                output_filename = os.path.join(
                    self.output_dir, os.path.basename(filename).split(".")[0] + ".pkl"
                )
                for label in label_columns:
                    self.saved_results[filename][label] = np.stack(
                        self.saved_results[filename][label]
                    )
    ...
    def _append_root_dir_to_filenames_and_check_files_exist(df, root_dir):
        ...
        for f in filenames:
            if not os.path.exists(f):
                raise ValueError(
                    f"<EvalVideoDataset>: file={f} does not exists. "
    ...
    def _generate_clips_metadata(df, clip_length, frame_rate, stride):
        ...
        for i, row in df.iterrows():
            total_frames_after_resampling = int(
                row["video-frames"] * (float(frame_rate) / row["fps"])
            )
            idxs = EvalVideoDataset._resample_video_idx(
                total_frames_after_resampling, row["fps"], frame_rate
            )
            ...
            clip_t_start = list(
                frame_idxs[np.arange(0, frame_idxs.shape[0] - clip_length + 1, stride)]
                / row["fps"]
            )
