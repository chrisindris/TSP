...
def main(args):
    ...
    print(
        f"shard-id: {args.shard_id + 1} out of {args.num_shards}, "
    ...
    metadata_df["is-computed-already"] = metadata_df["filename"].map(
        lambda f: os.path.exists(
            os.path.join(args.output_dir, os.path.basename(f).split(".")[0] + ".pkl")
        )
    )
    metadata_df = metadata_df[metadata_df["is-computed-already"] == False].reset_index(
        drop=True
    )
    print(
        f"Number of videos to process after excluding the ones already computed on disk: {len(metadata_df)}"
    )
    ...
    if args.local_checkpoint:
        ...
        pretrained_state_dict = torch.load(args.local_checkpoint, map_location="cpu")[
            "model"
        ]
    else:
        ...
        pretrained_state_dict = torch.hub.load_state_dict_from_url(
            MODEL_URLS[args.released_checkpoint],
            progress=True,
            check_hash=True,
            map_location="cpu",
        )["model"]
    ...
    model = Model(
        backbone=args.backbone, num_classes=[1], num_heads=1, concat_gvf=False
    )
    model.to(device)

    # remove the classifier layers from the pretrained model and load the backbone weights
    pretrained_state_dict = {
        k: v for k, v in pretrained_state_dict.items() if "fc" not in k
    }
